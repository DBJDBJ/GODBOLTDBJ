...
typedef struct queue_##TYPE##_##INT_TYPE { \
 ...
 \
void queue_##TYPE##_##INT_TYPE##_init(queue_##TYPE##_##INT_TYPE *queue_in) { \
...
void queue_##TYPE##_##INT_TYPE##_deinit(queue_##TYPE##_##INT_TYPE *queue_in) { \
    free(queue_in->buffer); \
} \
void queue_##TYPE##_##INT_TYPE##_should_reallocate(queue_##TYPE##_##INT_TYPE *q) { \
    if (q->capacity == q->size) { \
        if (q->capacity == 0) q->capacity = 2; \
        else q->capacity *= 2; \
        \
        TYPE* buffer_new = (TYPE*)malloc(sizeof(TYPE) * q->capacity); \
        if (buffer_new == NULL) abort(); \
        \
...
void queue_##TYPE##_##INT_TYPE##_push_back(queue_##TYPE##_##INT_TYPE *q, const TYPE *data) { \
...
void queue_##TYPE##_##INT_TYPE##_push_back_literal(queue_##TYPE##_##INT_TYPE *q, TYPE data) { \
    queue_##TYPE##_##INT_TYPE##_should_reallocate(q); \
    \
    q->buffer[q->back] = data; \
    q->back = (q->back + 1) % q->capacity; \
...
TYPE *queue_##TYPE##_##INT_TYPE##_emplace_back(queue_##TYPE##_##INT_TYPE *q) { \
    queue_##TYPE##_##INT_TYPE##_should_reallocate(q); \
    \
...
TYPE *queue_##TYPE##_##INT_TYPE##_iter(queue_##TYPE##_##INT_TYPE *q, INT_TYPE i) { \
    assert(i >= 0 && i < q->size); \
    \
