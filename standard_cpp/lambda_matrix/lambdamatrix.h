...
class memory_keeper {
  ...
  public:
    ...
    ~memory_keeper() noexcept { release(); }
...
template <size_t W, size_t H> struct setup {
    static constexpr size_t width_ = W,   // thus 0 .. 9
        height_ = H,                      //  thus 0 .. 9
...
// 	heap_block_type() noexcept : block((T(*)[N])calloc(1, sizeof(T[N]))) {
// assert(block); } 	~heap_block_type() noexcept
// 	{
// 		if (block)
// 		{
// 			free(block);
...
// 	}
// 	heap_block_type(heap_block_type const &other_) noexcept :
// block((T(*)[N])calloc(1, sizeof(T[N])))
// 	{
// 		assert(block);
// 		copy(*this, other_);
// 	}
